...
var cacheVolumeregions = [];
regionlist.forEach(filtercacheVolumeregions);
//this function build an array based on which regions have 'cachevolumes' set to true
function filtercacheVolumeregions(item, index) {
    if (item.cachevolumes == true) {
        cacheVolumeregions.push(item)
    }
};
...
//this function build an array based on which regions have 'cmk' set to true
...
var elasticZRSRegions = [];
regionlist.forEach(filterElasticZRSRegions);
//this function build an array based on which regions have 'elasticzrs' set to true
function filterElasticZRSRegions(item, index) {
    if (item.elasticzrs == true) {
        elasticZRSRegions.push(item)
    }
};
...
function initMap() {
    ...
    document.getElementById("cacheVolumeRegionCount").innerHTML = '<a target="_blank" href="https://learn.microsoft.com/azure/azure-netapp-files/azure-netapp-files-network-topologies#supported-regions">' + cacheVolumeregions.length + '</a>'
...
function updateMap() {
    ...
    if (document.getElementById("cachevolumes").checked) {
        cacheVolumeregions = [];
