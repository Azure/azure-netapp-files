...
function getResults(convert) {
    ...
    if (isNaN(change_rate) || change_rate <= 0 || change_rate > 100) {
        ...
        document.getElementById("init_bkup_monthly_cell").innerText = (volume_in_gb * backup_rate).toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,');;
        ...
        document.getElementById("inc_bkup_monthly_cell").innerText = ((monthly_delta / 30) * backup_rate).toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,');;
